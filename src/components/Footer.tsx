...
const WALK_CAT_DURATION_MS = 4800;

function WalkingCatSilhouette({ animationKey }: { animationKey: number }) {
  return (
    <div
      key={animationKey}
      className="walk-cat-scene pointer-events-none fixed inset-x-0 bottom-14 z-50 h-20 overflow-visible"
      aria-hidden
    >
      <div className="walk-cat-traveler">
        <div className="walk-cat-bob">
          <svg
            viewBox="0 0 400 380"
            width="72"
            height="68"
            ...
            aria-hidden
          >
            {/* Public domain side-profile cat silhouette (Wikimedia Commons) */}
            <path
...
export default function Footer() {
  const [showWalkingCat, setShowWalkingCat] = useState(false);
  const [animationKey, setAnimationKey] = useState(0);
  const hideTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  const triggerWalkingCat = useCallback(() => {
    if (hideTimeoutRef.current) {
      clearTimeout(hideTimeoutRef.current);
    }

    setAnimationKey((key) => key + 1);
    setShowWalkingCat(true);

    hideTimeoutRef.current = setTimeout(() => {
      setShowWalkingCat(false);
      hideTimeoutRef.current = null;
    }, WALK_CAT_DURATION_MS);
  }, []);
    ...
    <>
      <footer className="mx-auto mt-32 max-w-3xl border-t border-border pt-8 font-mono text-xs text-muted">
        <p className="inline-flex flex-wrap items-center gap-x-1.5">
          ...
          <button
            type="button"
            onClick={triggerWalkingCat}
            aria-label="Hidden surprise"
            className="inline-flex translate-y-px items-center border-0 bg-transparent p-0 text-muted/55 transition-all duration-200 hover:scale-110 hover:text-accent hover:drop-shadow-[0_0_8px_color-mix(in_srgb,var(--accent)_40%,transparent)]"
          >
            <Cat size={15} strokeWidth={1.75} aria-hidden />
          </button>
          <span className="text-accent">2026</span>
        </p>
      </footer>

      {showWalkingCat && <WalkingCatSilhouette animationKey={animationKey} />}
    </>
  );
}
